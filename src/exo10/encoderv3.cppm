...
export module lzw.encoderv3;

import lzw.encoder;

export namespace lzw {

    // Troisième version en utilisant std::unordered_map
    class EncoderV3 : public Encoder {
    public:
        ...
        void Decode(std::ifstream& in, std::ofstream& out) override {
            std::unordered_map<keycode_t, std::string> dictionary;
        ...
        void Encode(std::ifstream& in, std::ofstream& out) override {
            std::unordered_map<std::string, keycode_t> dictionary;
            const auto reset_dictionary = [&dictionary, this] {
                dictionary.clear();
                for(keycode_t i = 0; i < firstFreeKeyCode; i++) {
